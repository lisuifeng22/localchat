...
async def update_config(update: ConfigUpdate):
    ...
    if update.default_prompt is not None:
        config.data.setdefault("image", {}).setdefault("sd_webui", {})["default_prompt"] = update.default_prompt
    ...
    global provider, audio_processor
    provider = None
    get_provider()
    audio_processor = AudioProcessor(voice_config=config.get_voice_config())
    return {"ok": True}
...
async def _stream_chat_response(context: list[ChatMessage]):
    ...
    async def event_stream():
        full_response = ""
        try:
            async for chunk in p.chat_stream(context, temperature=config.temperature, max_tokens=config.max_tokens):
                if chunk.startswith("\n[ERROR]"):
