...
BACKUP_SUFFIX = ".sd_model_selector_v6_8.bak"

SERVER_REMOVE_PATTERNS = [
    r"\n?# ── Stable Diffusion Model Selector v6\.8[\s\S]*?# ── end Stable Diffusion Model Selector v6\.8[^\n]*\n?",
...
HTML_BLOCK = """
<!-- Stable Diffusion model selector v6.8 -->
<div class="form-row sd-model-selector-v68" style="display:grid;grid-template-columns:1fr;gap:12px;margin:12px 0;">
  <div class="form-group">
    ...
    <div style="display:flex;gap:8px;align-items:center;">
      ...
      <button type="button" onclick="loadSdModelListV68()" title="刷新模型列表">刷新</button>
    </div>
...
FRONTEND_JS = """
// ── Stable Diffusion model selector v6.8 ───────────────────────────────────
async function fetchJsonV68(url, options) {
  const res = await fetch(url, options);
  const text = await res.text();
  try {
    return JSON.parse(text);
  } catch (e) {
    throw new Error(`${url} 没有返回 JSON，HTTP ${res.status}，前 160 字：${text.slice(0, 160)}`);
  }
}

async function loadSdModelListV68(selectedValue) {
  ...
  if (hint) hint.textContent = '正在请求 localchat 后端 /api/sd/models ...';

  try {
    const data = await fetchJsonV68('/api/sd/models?_=' + Date.now());
    ...
    if (!models.length) {
      ...
      opt.selected = true;
      sel.appendChild(opt);
      if (hint) hint.textContent = `后端已连通，但模型数量为 0；SD 地址：${data.base_url || ''}`;
      return;
    }
...
async function loadSdModelSettingV68() {
  let selected = '';
  try {
    const data = await fetchJsonV68('/api/sd/model-setting?_=' + Date.now());
    selected = data.model_checkpoint || '';
  } catch (e) {
    console.warn('读取 SD 模型设置失败', e);
  }
  await loadSdModelListV68(selected);
}

async function saveSdModelSettingV68() {
  const sel = document.getElementById('selSdModel');
  const hint = document.getElementById('sdModelHint');
  if (!sel) return;

  try {
    const data = await fetchJsonV68('/api/sd/model-setting', {
      method: 'POST',
...
function installSdModelHooksV68() {
  if (window.__sdModelHooksV68Installed) return;
  window.__sdModelHooksV68Installed = true;

  setTimeout(() => {
    if (document.getElementById('selSdModel')) loadSdModelSettingV68();
  }, 300);

  if (typeof window.showSettings === 'function' && !window.showSettings.__sdModelV68Wrapped) {
    ...
    const wrappedShowSettings = async function(...args) {
      ...
      setTimeout(() => loadSdModelSettingV68(), 50);
      return ret;
    };
    wrappedShowSettings.__sdModelV68Wrapped = true;
    window.showSettings = wrappedShowSettings;
  }

  if (typeof window.saveSettings === 'function' && !window.saveSettings.__sdModelV68Wrapped) {
    const oldSaveSettings = window.saveSettings;
    const wrappedSaveSettings = async function(...args) {
      await saveSdModelSettingV68();
      return await oldSaveSettings.apply(this, args);
    };
    wrappedSaveSettings.__sdModelV68Wrapped = true;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', installSdModelHooksV68);
} else {
  installSdModelHooksV68();
}
// ── end Stable Diffusion model selector v6.8 ───────────────────────────────
"""
...
def find_fastapi_app_call_end(text: str) -> int:
...
def patch_server() -> None:
    ...
    insert_at = find_fastapi_app_call_end(text)
    if insert_at == -1:
        fail("没有找到 app = FastAPI(...)，无法安全插入后端接口")

    text = text[:insert_at].rstrip() + "\n\n" + SERVER_SNIPPET.strip() + "\n\n" + text[insert_at:].lstrip()
    write_text(SERVER, text)
    compile_python(SERVER)
    print("[完成] 后端接口已强制插入到 app = FastAPI(...) 后面")
    print("[提示] 重启后测试：http://127.0.0.1:8000/api/sd/models")


def remove_old_html_blocks(html: str) -> str:
    for ver in ("v6.8", "v6.7", "v6.6"):
        html = re.sub(
            rf"\n?<!-- Stable Diffusion model selector {re.escape(ver)} -->[\s\S]*?<!-- end Stable Diffusion model selector {re.escape(ver)} -->\n?",
            "\n",
            html,
            flags=re.MULTILINE,
        )
    ...
    for cls in ["sd-model-selector-v68", "sd-model-selector-v67", "sd-model-selector-v66"]:
        while cls in html:
            p = html.find(cls)
            start = html.rfind("<div", 0, p)
...
def main() -> None:
    ...
    print("[开始] v6.8 修复：API 路由顺序 + 前端位置")
    patch_server()
    patch_index()
    ...
    print("2. 浏览器打开：http://127.0.0.1:8000/api/sd/models")
    print("   必须看到 {\"ok\": true, \"models\": [...]} 这种 JSON")
    print("3. 浏览器 Ctrl + F5")
    print("4. 打开设置页查看 Stable Diffusion 模型下拉框")
