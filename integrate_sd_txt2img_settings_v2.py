...
# -*- coding: utf-8 -*-
"""
LocalChat v5 Stable Diffusion 文生图参数完整集成脚本 v2

把本文件放到 localchat 项目根目录运行：
    python integrate_sd_txt2img_settings_v2.py
...
重点：这版会强制检查后端替换是否成功。如果 server.py 没改成功，会直接报错退出，不会只改前端。
"""
...
def die(msg: str) -> None:
    print(f"[SD集成v2][错误] {msg}", file=sys.stderr)
    sys.exit(1)


def read_text(path: Path) -> str:
    if not path.exists():
        die(f"找不到文件：{path}\n请把脚本放到 localchat 项目根目录运行，也就是 config.py 所在目录。")
...
def backup(path: Path) -> None:
    ...
    log(f"备份 {path.relative_to(ROOT)} -> {bak.name}")


def must_replace(text: str, pattern: str, repl: str, desc: str, flags: int = re.S) -> str:
    new_text, n = re.subn(pattern, repl, text, count=1, flags=flags)
    if n != 1:
        die(f"后端/文件替换失败：{desc}\n没有找到匹配位置。请确认你运行的是 localchat v5，并且文件没有被手动改乱。")
    log(f"完成：{desc}")
    return new_text


def maybe_replace(text: str, pattern: str, repl: str, desc: str, flags: int = re.S) -> tuple[str, bool]:
    new_text, n = re.subn(pattern, repl, text, count=1, flags=flags)
    if n == 1:
        log(f"完成：{desc}")
        return new_text, True
    log(f"跳过：{desc}（未找到匹配位置）")
    return text, False
...
def patch_config() -> None:
    ...
    text, ok = maybe_replace(
        text,
        r'("cfg_scale"\s*:\s*7\.0\s*,)',
        r'\1' + insert_after_cfg,
        "config.py 插入 SD 默认参数",
    )
    if ok:
...
class SDWebUIProvider(ImageProvider):
    """Generates images via AUTOMATIC1111 Stable Diffusion WebUI API."""
...
'''
    ...
    log("providers/image.py 已整体替换，后端 SD payload 已接入")
...
def patch_server() -> None:
    ...
    new_models = r'''class DrawRequest(BaseModel):
    prompt: str
...
'''
    text = must_replace(
        ...
        new_models,
        "server.py 后端请求模型 DrawRequest / ConfigUpdate",
    )

    new_get_config = r'''@app.get("/api/config")
...
'''
    text = must_replace(
        ...
        new_get_config,
        "server.py /api/config 返回 image 配置",
    )

    new_update_config = r'''@app.post("/api/config")
...
'''
    text = must_replace(
        ...
        new_update_config,
        "server.py /api/config 保存 image 配置并重置 image_provider",
    )

    sd_meta = r'''@app.get("/api/sd/meta")
...
'''
    if '"/api/sd/meta"' not in text:
        marker = '@app.get("/api/models")'
        if marker not in text:
            die("找不到 @app.get(\"/api/models\")，无法插入 /api/sd/meta")
        text = text.replace(marker, sd_meta + marker, 1)
        log("完成：server.py 新增 /api/sd/meta 后端接口")
    else:
        log("server.py 已有 /api/sd/meta，跳过新增")

    new_draw = r'''@app.post("/api/draw")
...
'''
    text = must_replace(
        ...
        new_draw,
        "server.py /api/draw 透传 SD 参数到 _generate_image",
    )
    ...
    required_markers = [
        ...
        'img_cfg["sd_model_checkpoint"]',
        'kwargs["clip_skip"]',
        '"image": {',
    ]
    missing = [m for m in required_markers if m not in verify]
    if missing:
        die("server.py 后端校验失败，缺少：" + ", ".join(missing))
    log("server.py 后端校验通过")
...
.sd-inline { display: flex; align-items: center; gap: 8px; }
.sd-inline input[type=\"checkbox\"] { width: auto; }
"""
    if ".sd-grid" not in text:
        text = text.replace("</style>", css + "\n</style>", 1)
        log("完成：index.html 新增 SD 样式")
    else:
        log("index.html 已有 SD 样式，跳过")
...
<div class="sd-grid">
  ...
  <div class="sd-field sd-field-full">
    <label class="sd-inline"><input type="checkbox" id="chkHiresFix"><span>高分辨率修复 (Hires. fix)</span></label>
  </div>
  <div class="sd-field">
...
'''
    if "SD_TXT2IMG_SETTINGS_START" not in text:
        text = must_replace(
            text,
            r'(<textarea[^>]+id=["\']inputDefaultNegPrompt["\'][\s\S]*?</textarea>)',
            r'\1' + html,
            "index.html 在反向提示词下面插入 SD 参数 UI",
        )
    else:
        log("index.html 已有 SD 参数 UI，跳过")

    js = r'''
// SD_TXT2IMG_SCRIPT_START
function sdShowError(msg) { if (typeof showError === 'function') showError(msg); else alert(msg); }
function intVal(id, fallback = null) { const el = document.getElementById(id); const v = el ? el.value : ''; if (v === '' || v == null) return fallback; const n = parseInt(v, 10); return Number.isNaN(n) ? fallback : n; }
function floatVal(id, fallback = null) { const el = document.getElementById(id); const v = el ? el.value : ''; if (v === '' || v == null) return fallback; const n = parseFloat(v); return Number.isNaN(n) ? fallback : n; }
function setSelectOptions(id, items, selectedValue = '', prependNone = false, noneLabel = 'None') {
  const el = document.getElementById(id); if (!el) return;
  let list = Array.isArray(items) ? [...items] : [];
  if (prependNone && !list.includes('None')) list.unshift('None');
  if (selectedValue && !list.includes(selectedValue)) list.unshift(selectedValue);
  el.innerHTML = '';
  if (!list.length) { const op = document.createElement('option'); op.value = ''; op.textContent = '无可用数据'; el.appendChild(op); return; }
  list.forEach(item => { const op = document.createElement('option'); op.value = item; op.textContent = item || noneLabel; if (item === selectedValue) op.selected = true; el.appendChild(op); });
}
...
async function showSettings() {
  try {
    ...
    try { meta = await api('GET', '/api/sd/meta'); } catch (e) { console.warn('加载 SD 元数据失败：', e); }
    const setVal = (id, value) => { const el = document.getElementById(id); if (el) el.value = value ?? ''; };
    setVal('selProvider', cfg.provider || 'openai');
    setVal('inputKey', cfg.openai?.api_key || ''); setVal('inputEndpoint', cfg.openai?.base_url || ''); setVal('inputModel', cfg.openai?.model || '');
    setVal('inputAnthropicKey', cfg.anthropic?.api_key || ''); setVal('inputAnthropicModel', cfg.anthropic?.model || '');
    setVal('inputTemp', cfg.temperature ?? 0.7); setVal('inputDefaultPrompt', cfg.default_prompt || ''); setVal('inputDefaultNegPrompt', cfg.default_negative_prompt || '');
    const img = cfg.image || {};
    ...
    setVal('inputClipSkip', img.clip_skip ?? 1); setVal('inputImageSteps', img.steps ?? 20); setVal('inputImageCfgScale', img.cfg_scale ?? 7);
    setVal('inputImageWidth', img.width ?? 512); setVal('inputImageHeight', img.height ?? 512); setVal('inputImageBatchCount', img.n_iter ?? 1); setVal('inputImageBatchSize', img.batch_size ?? 1);
    setVal('inputRefinerCheckpoint', img.refiner_checkpoint || ''); setVal('inputRefinerSwitchAt', img.refiner_switch_at ?? 0.8);
    const hires = document.getElementById('chkHiresFix'); if (hires) hires.checked = !!img.enable_hr;
    setVal('selTtsProvider', cfg.voice?.tts_provider || 'local'); setVal('selSttProvider', cfg.voice?.stt_provider || 'local');
    setVal('inputVolcApiKey', cfg.voice?.volcengine?.api_key || ''); setVal('inputVolcResourceId', cfg.voice?.volcengine?.resource_id || 'auto'); setVal('inputVolcVoiceType', cfg.voice?.volcengine?.voice_type || 'zh_female_vv_uranus_bigtts');
    onProviderChange(); onTtsProviderChange();
    document.getElementById('settingsModal')?.classList.add('show');
  } catch (e) { sdShowError(`加载设置失败：${e.message}`); }
}
async function saveSettings() {
  try {
    ...
    const payload = {
      ...
      image_steps: intVal('inputImageSteps', 20), image_width: intVal('inputImageWidth', 512), image_height: intVal('inputImageHeight', 512),
      image_cfg_scale: floatVal('inputImageCfgScale', 7), image_batch_size: intVal('inputImageBatchSize', 1), image_n_iter: intVal('inputImageBatchCount', 1),
      image_enable_hr: !!document.getElementById('chkHiresFix')?.checked,
      image_refiner_checkpoint: document.getElementById('inputRefinerCheckpoint')?.value.trim() || '', image_refiner_switch_at: floatVal('inputRefinerSwitchAt', 0.8),
      sd_model_checkpoint: document.getElementById('selSdModel')?.value || '', sd_vae: document.getElementById('selSdVae')?.value || 'None', clip_skip: intVal('inputClipSkip', 1),
      tts_provider: document.getElementById('selTtsProvider')?.value || 'local', stt_provider: document.getElementById('selSttProvider')?.value || 'local',
      volcengine_api_key: document.getElementById('inputVolcApiKey')?.value || '', volcengine_resource_id: document.getElementById('inputVolcResourceId')?.value || '', volcengine_voice_type: document.getElementById('inputVolcVoiceType')?.value || '',
    };
    if (provider === 'anthropic') { payload.api_key = document.getElementById('inputAnthropicKey')?.value || ''; payload.model = document.getElementById('inputAnthropicModel')?.value.trim() || ''; }
    else { payload.api_key = document.getElementById('inputKey')?.value || ''; payload.base_url = document.getElementById('inputEndpoint')?.value.trim() || ''; payload.model = document.getElementById('inputModel')?.value.trim() || ''; }
    await api('POST', '/api/config', payload);
    closeModal('settingsModal'); if (typeof loadStatus === 'function') await loadStatus();
  } catch (e) { sdShowError(`保存设置失败：${e.message}`); }
}
async function drawImage() {
  const input = document.getElementById('chatInput'); const drawBtn = document.getElementById('drawBtn'); const text = input?.value.trim();
  if (!text || streaming) return;
  input.value = ''; input.style.height = 'auto'; appendMessage('user', `生图：${text}`, false); appendMessage('assistant', '正在生成图片...', true);
  streaming = true; if (drawBtn) drawBtn.disabled = true;
  try {
    ...
    if (data.enhanced_prompt && data.enhanced_prompt !== data.prompt) content += `\n\n> ✨ ${data.enhanced_prompt}`;
    appendMessage('assistant', content, true); finalizeStreaming();
  } catch (e) { finalizeStreaming(); sdShowError(e.message); }
  finally { streaming = false; if (drawBtn) drawBtn.disabled = false; if (typeof loadSessions === 'function') await loadSessions(); if (typeof loadStatus === 'function') await loadStatus(); input?.focus(); }
}
// SD_TXT2IMG_SCRIPT_END
'''
    text = re.sub(r'\n?// SD_TXT2IMG_SCRIPT_START[\s\S]*?// SD_TXT2IMG_SCRIPT_END\n?', '\n', text, flags=re.S)
    if "</script>" not in text:
        die("index.html 找不到 </script>，无法插入前端脚本")
    text = text.replace("</script>", js + "\n</script>", 1)
    log("完成：index.html 新增/刷新 SD 前端脚本")
...
def main() -> None:
    ...
    patch_server()
    patch_index()
    log("全部完成。现在后端、前端都已修改。")
    log("请重启 LocalChat 服务。")
    log("验证1：http://127.0.0.1:8000/api/config 应该能看到 image 字段。")
    log("验证2：http://127.0.0.1:8000/api/sd/meta 应该能看到 models / vaes / samplers / schedulers。")
    log("验证3：Stable Diffusion WebUI 必须用 --api 启动，否则模型列表会空。")
